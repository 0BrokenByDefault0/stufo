...
final class SmokeTests: XCTestCase {
    func testCoreJourneyAndPersistence() {
        ...
        app.buttons.containing(.staticText, identifier: "Record a clean vocal").firstMatch.tap()
        XCTAssertTrue(app.buttons["workflowStep0"].waitForExistence(timeout: 5))
        app.buttons["workflowStep0"].tap()
        XCTAssertTrue(app.buttons["workflowStep0"].label.contains("complete"))
        capture("04-session")
        app.tabBars.buttons["Ask"].tap()
        app.buttons["How do I throw a delay?"].tap()
